...
class TRON1SF_EE(LeggedRobotEE):
    ...
    def compute_observations(self):
        ...
        if self.cfg.asset.obtain_link_contact_states:
            single_critic_obs = torch.cat(
                (
                    ...
                    self.simulator.link_contact_states,  # contact states of base, abad, hip, knee and ankle (1+2+2+2+2)=9
                ),
                dim=-1,
            )
        if self.cfg.terrain.measure_heights: # 49
            heights = torch.clip(self.simulator.base_pos[:, 2].unsqueeze(
                1) - 0.7 - self.simulator.measured_heights, -1, 1.) * self.obs_scales.height_measurements
        ...
        self.estimator_labels_buf = torch.cat((
            ...
            self.simulator.link_contact_states, # contact states of base, abad, hip, knee and ankle (2+2+2+2+1)=9
    ...
    def _reward_ankle_torque_limits(self):
        torque_limit = torch.cat((self.simulator.torque_limits[3].view(1) * self.cfg.rewards.soft_torque_limit,
                                  self.simulator.torque_limits[7].view(1) * self.cfg.rewards.soft_torque_limit),
                                 dim=-1, )
        torque = torch.cat((self.simulator.torques[:, 3].view(self.num_envs, 1),
                            self.simulator.torques[:, 7].view(self.num_envs, 1)), dim=-1)
        return torch.sum(
