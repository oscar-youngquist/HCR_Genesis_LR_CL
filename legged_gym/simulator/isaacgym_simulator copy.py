...
if SIMULATOR == "isaacgym":
    ...
    try:
        ...
        trimesh = None
    try:
        from legged_gym.warp.warp_cam import WarpCam  # type: ignore
    except Exception:  # pragma: no cover
        WarpCam = None
...
class IsaacGymSimulator(Simulator):
    def __init__(self, cfg, sim_params: dict, sim_device: str = "cuda:0", headless: bool = False):
        ...
        if self._cfg.sensor.use_warp:
            ...
            if wp is None or WarpCam is None:
                raise ImportError(
                    "cfg.sensor.use_warp=True requires the `warp` dependency and `legged_gym.warp` modules. "
                    "Please install warp in this python environment."
                )
            wp.init()
    ...
    def _create_envs(self):
        ...
        # Align action order with cfg.asset.dof_names when provided.
        # This is critical for robots whose URDF dof order differs from desired control order.
        cfg_dof_names = getattr(self._cfg.asset, "dof_names", None)
        if (
            isinstance(cfg_dof_names, (list, tuple))
            and len(cfg_dof_names) == self._num_actions
            and all(name in self._dof_names for name in cfg_dof_names)
        ):
            self._dof_indices = [self._dof_names.index(name) for name in cfg_dof_names]
        else:
            # Fallback: assume URDF dof order matches action order.
            self._dof_indices = list(range(len(self._dof_names)))

        if len(self._dof_indices) != self._num_actions:
            raise ValueError(
                f"IsaacGym dof mapping mismatch: len(dof_indices)={len(self._dof_indices)} "
                f"but cfg.env.num_actions={self._num_actions}. "
                f"Please set cfg.asset.dof_names to a list of length {self._num_actions} "
                f"with names from the URDF dofs."
            )
    ...
    def _create_trimesh(self):
        ...
        if trimesh is None:
            raise ImportError(
                "Terrain mesh_type='trimesh' requires the `trimesh` package. "
                "Install it in your IsaacGym python environment (e.g. `pip install trimesh`)."
            )
