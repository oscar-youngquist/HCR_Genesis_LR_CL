...
class IsaacLabSimulator(Simulator):
    ...
    def _create_envs(self):
        ...
        print(f"Termination contact link indices: {self.termination_contact_indices}")
        self.penalized_contact_indices = find_link_indices(
            self.cfg.asset.penalize_contacts_on)
        print(f"Penalized contact link indices: {self.penalized_contact_indices}")
        self.feet_names = [
            link for link in self.robot.body_names if self.cfg.asset.foot_name in link
        ]
        self.feet_indices = find_link_indices(self.feet_names)
        print(f"feet names: {self.feet_names}")
        assert len(self.feet_indices) > 0
